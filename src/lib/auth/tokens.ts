...
export default async function resolveAccessToken(
  payload: regenerateTokenInput
) {
  ...
  // not missing.
    ...
    // it isn't nesseccary to add "err" inside catch.
    else if (res.message === 'expired') {
      const refreshToken = getAuthToken('jwtRefreshToken')
