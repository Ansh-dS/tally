...
export function cleanResponse<T = unknown>(
    ...
    const arr = Object.entries(payload).filter(([_, value]) => {
...
export function errorResponse<T = unknown>(data: Partial<ApiResponse<T>>): Partial<ApiResponse<T>> {
    // building the payload
...
export function failedResponse<T = unknown>(data: Partial<ApiResponse<T>>): Partial<ApiResponse<T>> {
    // building the payload
    const payload: Partial<ApiResponse<T>> = {
        status: 'failed',
...
export function successResponse<T = unknown>(data: Partial<ApiResponse<T>>): Partial<ApiResponse<T>> {
    // building the payload
    const payload: Partial<ApiResponse<T>> = {
        status: 'success',
