...
export function SubmissionSwitcher({ formAttributes, formId }: formData) {
  ...
  if (typeof hashedPassword === 'string' && !isValidPass) {
    return <CheckFormPass hashedPassword={hashedPassword}></CheckFormPass>
  }
  ...
  return (
    <ResponsePage formData={formData} formId={formId} pageName="submitPage" />
  )
}

export function SubmissionWithPassProvider({
  formAttributes,
  formId,
}: formData) {
  return (
