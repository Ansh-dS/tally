...
export function ResponsePage({
    ...
    const onSubmit = async (data: ResponseRow) => {
        ...
        if (pageName === 'submitPage') {
            ...
            const isSubmitted = await submitUserResponse({ answers: data, formId: formId ?? '' })
    ...
    return (
        ...
        <Stack as="form" onSubmit={handleSubmit(onSubmit)} align={"center"} justify={"start"} className="min-h-screen w-full bg-surface-base px-l text-fg-primary">
            ...
            <Box className="flex-1 w-full max-w-170 min-h-125 flex flex-col px-10 py-12 mt-8 mb-32 border-border-default/15 shadow-popout rounded-lg bg-surface-raised text-fg-primary">
                ...
                <Stack className="gap-12 px-l"  >
                    <AnimatePresence mode="wait">
                        ...
                        >
                            {blocks.length !== 0 && currentBlocks.map((block, index) => (
