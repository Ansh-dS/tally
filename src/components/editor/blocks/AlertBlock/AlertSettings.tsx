...
export function AlertSettings({ block, updateBlock }: AlertSettingsProps) {
  return (
    <Stack gap="lg" className="p-m">
      <Stack gap="sm">
        <Text variant="label" weight="semibold">Alert Message</Text>
        <TextArea 
         
          onChange={(e) => updateBlock(block.id, { data: { ...block.data, placeholder: e.target.value } })}
      ...
      <Stack gap="sm">
        <Text variant="label" weight="semibold">Severity Level</Text>
        <Select 
          value={block.data?.severity || 'info'}
          onChange={(value) => updateBlock(block.id, { 
            data: { ...block.data, severity: value } 
          })}
          options={[
