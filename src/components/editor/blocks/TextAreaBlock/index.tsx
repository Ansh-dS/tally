    ...
    <Box className={`w-full ${isOverlay ? 'opacity-80' : ''}`}>
      <Stack gap="sm">
        <Stack direction={"horizontal"}>
          <EditableTitle block={block} isOverlay={isOverlay} updateBlock={updateBlock} defaultText="Long Text Question" />
          {block?.required && <Text color={'accent'} variant={"caption"} className='ml-s'>*</Text>}
        </Stack>  
        <TextArea placeholder={block?.data?.placeholder || "Write your answer here..."} readOnly rows={3} className="pointer-events-none opacity-60" />
      </Stack>
    </Box>
  ),
